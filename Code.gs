...
function myFunction(){
  ...
  var startTime = Utilities.formatDate(new Date(), timezone, "HH:mm yyyy-MM-dd");
   Browser.msgBox('time is: '+startTime) 
}
...
function onOpen(){
  ...
  ui.createMenu('Auto Vjudge')
       ...
       //.addItem('All Vjudge Contest Update', 'vjudgeAllContestUpdate')
       .addToUi();
...
function vjudgeAllContestUpdate()
{
  timezone = "GMT+" + new Date().getTimezoneOffset()/60
  var startTime = Utilities.formatDate(new Date(), timezone, "HH:mm yyyy-MM-dd");
  var col = 1;
  ...
  var r =0;
  for(r=currentChosenCellRow();sheet.getRange(r, 1).getValue()!="";r++)
  {
    var row = r;
    var formula = sheet.getRange(row, 1).getFormula();
  //var url = getUrlFromForumla(formula);
  ...
  var endtTime = Utilities.formatDate(new Date(), timezone, "HH:mm yyyy-MM-dd");
  Browser.msgBox('Finished Running Script For All Contest.\nScript starts at:'+startTime+'\nEnds at: '+endtTime+'\nLast Row is '+ r);
}
  ...
  if (extracted != null && extracted.length >= 2){
    ...
    if(strs[2]=='vjudge.net'&&strs[3]=="contest"&&strs[4]!=''){
      var contestID = strs[4].split("#")
       //Browser.msgBox('Contest link is: '+contestUrl);
      return contestID[0];
    }
